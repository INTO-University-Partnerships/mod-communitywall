...
require_once __DIR__ . '/../src/mod_communitywall_restore_controller.php';

/**
 * @see http://docs.moodle.org/dev/Restore_2.0_for_developers
 */
class mod_communitywall_restore_test extends advanced_testcase {
    ...
    /**
     * @var mod_communitywall_restore_controller
     */
    protected $_cut;
    ...
    protected function setUp() {
        ...
        $this->_cut = new mod_communitywall_restore_controller(
            $folder,
            $this->_courseid,
    ...
    public function test_execute_plan_restores_module() {
        ...
        $this->assertEquals(1, $DB->count_records('communitywall'));
        $data = (array)$DB->get_record('communitywall', array(), '*', MUST_EXIST);
        $this->assertSame('Wall 001', $data['name']);
        $this->assertEquals(1, $data['closed']);
        ...
        $this->assertEquals(0, $DB->count_records('communitywall_wall'));
